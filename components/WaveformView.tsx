      ...
      {bars ? (
            ...
            return (
              <View
                ...
                style={{
                  ...
                  backgroundColor: inRange ? colors.accent : colors.border,
                  borderRadius: 1,
                }}
              />
            );
        ...
        <View testID="waveform-fallback" style={styles.bars}>
          {Array.from({ length: 60 }).map((_, i) => {
            ...
            const inRange = sec >= range.start && sec <= range.end;
            return (
              <View key={i} style={{ flex: 1, marginHorizontal: 0.5, height: (height - 8) * 0.4, backgroundColor: inRange ? colors.accent : colors.border, borderRadius: 1 }} />
            );
          })}
        </View>
      )}
...
const makeStyles = (colors: ColorTokens) => StyleSheet.create({
  ...
  dim: { position: 'absolute', top: 0, bottom: 0, backgroundColor: colors.overlay },
  playhead: { position: 'absolute', top: 0, bottom: 0, width: 2, backgroundColor: colors.star },
  handle: { position: 'absolute', top: 0, bottom: 0, width: HANDLE_W, backgroundColor: colors.accent, opacity: 0.9, borderRadius: 4 },
});
